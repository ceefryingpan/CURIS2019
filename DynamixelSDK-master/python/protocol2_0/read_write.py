...
DXL_MINIMUM_POSITION_VALUE  = -50                           # Dynamixel will rotate between this value
DXL_MAXIMUM_POSITION_VALUE  = 50                            # and this value (note that the Dynamixel would not move when the position value is out of movable range. Check e-manual about the range of the Dynamixel you use.)
...
dxl_goal_position = [dxl_present_position + DXL_MINIMUM_POSITION_VALUE, dxl_present_position + DXL_MAXIMUM_POSITION_VALUE]
...
while 1:
    ...
    while 1:
        ...
        dxl_time = dynamixel.read2ByteTxRx(port_num, PROTOCOL_VERSION, DXL_ID, ADDR_PRO_REALTIME_TICK)
        dxl_comm_result = dynamixel.getLastTxRxResult(port_num, PROTOCOL_VERSION)
        dxl_error = dynamixel.getLastRxPacketError(port_num, PROTOCOL_VERSION)
        if dxl_comm_result != COMM_SUCCESS:
            print(dynamixel.getTxRxResult(PROTOCOL_VERSION, dxl_comm_result))
        ...
        print("[ID:%03d] GoalPos:%03d  PresPos:%03d  Device time %d Local time: %d" % (DXL_ID, dxl_goal_position[index], dxl_present_position, dxl_time, currentTimeuS))
